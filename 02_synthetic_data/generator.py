...
class DatasetGenerator:
    ...
    def _sample_dataset_params(self):
        ...
        # ── Derive t_max from m so that m * T <= max_feat_times_t ──────────
        max_mT = hp_d.max_feat_times_t
        t_max = min(hp_d.t_range[1], max_mT // max(1, m))
        t_min = min(hp_d.t_range[0], t_max)

        # Log-uniform T — biased strongly toward small values
        self.T = self._log_uniform_int(t_min, t_max)

        # Log-uniform n — biased strongly toward small values
        self.n_samples = self._log_uniform_int(hp_d.min_samples, hp_d.max_samples)

        self.apply_global_pooling = False
    ...
    def generate_dataset(self) -> dict | None:
        ...
        return {
            ...
            'T': T_obs,
            'n_samples': len(y),
            'apply_global_pooling': self.apply_global_pooling,
        }
