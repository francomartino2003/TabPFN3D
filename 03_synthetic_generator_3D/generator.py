...
2. DAG building (reusing 2D components)
3. Temporal propagation with noise/time/state inputs
4. Sequence sampling (IID, sliding window, mixed)
...
# 2D components via wrapper (avoids import conflicts)
...
class SyntheticDatasetGenerator3D:
    ...
    def __init__(
        ...
        Args:
            prior: Prior configuration for sampling. Uses default if None.
    ...
    def generate(self, config: Optional[DatasetConfig3D] = None) -> SyntheticDataset3D:
        ...
        # Step 1: Build DAG (reuse 2D component)
        dag, transformations = self._build_dag(config, dataset_rng)
        
        # Step 2: Create temporal input manager
    ...
    ) -> Tuple[DAG, Dict[Tuple[int, int], EdgeTransformation]]:
        """
        Build the causal DAG and transformations.
        
        Adapts the 2D DAG builder to work with 3D config.
        """
        # Create a minimal 2D-compatible config for DAG building
        from config import DatasetConfig3D
        
        # Build DAG using 2D builder
    ...
    def _generate_samples(
        ...
        if config.sample_mode == 'iid':
            # Generate independent sequences (all in parallel as batch)
            propagated = propagator.generate_iid_sequences(n_samples, T)
            # For IID, each row in the batch is an independent sequence
            # Extract one window per row
            samples = sampler.sample_iid_batch(propagated, n_samples)
            
        elif config.sample_mode == 'sliding_window':
            # Generate one long sequence, extract windows
            # Use batch size based on how many windows we can extract
            ...
            # Generate multiple long sequences
    ...
    def _apply_post_processing(
        ...
        Post-processing is applied per-feature across all samples and timesteps.
        """
        if X.size == 0:
            return X, y
        ...
        if config.apply_warping:
            ...
            for f in range(n_features):
                # Flatten feature across samples and time
...
if __name__ == "__main__":
    # Quick test
    print("Generating 3D synthetic dataset...")
    ...
    print(f"Target offset: {dataset.config.target_offset_type} ({dataset.config.target_offset})")
