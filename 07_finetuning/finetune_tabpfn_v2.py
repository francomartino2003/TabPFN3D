"""
Fine-tune TabPFN on Flattened Synthetic 3D Datasets (V2).

Uses the DAG-based generator from 11_final_generator.

Training approach:
- Generate synthetic 3D datasets on-the-fly using DAG generator (folder 11)
...
Usage:
    python finetune_tabpfn_v2.py --n-steps 1000 --eval-every 1
    python finetune_tabpfn_v2.py --debug  # Quick test run
"""
        ...
        with torch.no_grad():
            for data in datasets:
                ...
                try:
                    ...
                    clf.model_.load_state_dict(self.model.state_dict())
                    clf.model_.eval()
...
def save_eval_json(trainer: TabPFNFineTuner, log_dir: Path):
...
def train(config: FinetuneConfig, resume_from: Optional[str] = None):
    ...
    print("=" * 60)
    print("FINE-TUNING TABPFN  —  DAG Generator v5 (multi-root, dual-act)")
    print("  Series: pw(→d) + act1 + dilated_causal(K,D,d→1) + act2")
    print("=" * 60)
    ...
    for ds in real_datasets:
        print(f"    {ds['name']}: {ds['n_samples']} samp, "
              f"{ds['n_features']} feat, {ds['n_classes']} cls")
...
def main():
    parser = argparse.ArgumentParser(
        description='Fine-tune TabPFN with DAG generator (folder 11)')
