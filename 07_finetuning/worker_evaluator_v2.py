...
from worker_trainer_v2 import forward_single_dataset, deserialize_batch
...
def evaluate_real(model, clf, device, real_datasets):
    """Evaluate on real datasets with overlapping group expansion."""
    model.eval()
    results = []
...
def main():
    ...
    # Generate fixed synthetic eval batches (group_size=16 for overlap)
    cfg = FinetuneConfig(batch_size=args.batch_size, group_size=WINDOW)
