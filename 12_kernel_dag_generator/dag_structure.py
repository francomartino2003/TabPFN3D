"""
DAG structure builder for the kernel-DAG dataset generator.

Key differences from 11_final_generator:
  - Layer 0 (root) can contain SERIES nodes (GP-sampled), not just tabular/discrete.
  - Every internal series node is guaranteed ≥1 series parent (post-drop repair).
  - No time-index channels needed — temporal structure comes from series ancestry.
"""
...
def _log_uniform_int(rng: np.random.Generator, lo: int, hi: int) -> int:
    if lo == hi:
        return lo
    val = np.exp(rng.uniform(np.log(lo + 1), np.log(hi + 1))) - 1
...
def build_dag(
    ...
    Steps:
      ...
      2. Create root nodes (layer 0) — can be series, tabular, or discrete.
      3. Create hidden layers with random node types.
      3b. Guarantee ≥1 series root, ≥1 hidden series, ≥1 discrete anywhere.
      4. Wire fully-connected edges with random drops.
      4b. Guarantee every internal series node has ≥1 series parent.
      5. Assign roles.
      6. Prune below deepest role node.
      7. Re-index.
    """
    ...
    # ── 2. Root layer — series / tabular / discrete ───────────────────────
    ...
    # Guarantee ≥1 series root (needed so hidden series can inherit temporal info)
    root_nodes = [nodes[nid] for nid in layers[0]]
    if not any(n.node_type == 'series' for n in root_nodes):
        convert = root_nodes[rng.integers(0, len(root_nodes))]
        convert.node_type = 'series'

    # Guarantee ≥1 series in hidden layers
    ...
    # Guarantee ≥1 discrete node anywhere
    ...
    # ── 4b. Guarantee every series node has ≥1 series parent ──────────────
    # Walk layers top-down; for each series child missing a series parent,
    # find the nearest series node in any earlier layer and add the edge.

    for l_idx in range(1, len(layers)):
        for child_id in layers[l_idx]:
            child = nodes[child_id]
            if child.node_type != 'series':
                continue
            has_series_parent = any(
                nodes[p].node_type == 'series' for p in child.parents)
            if has_series_parent:
                continue

            # Find series nodes in earlier layers (prefer immediate parent layer)
            added = False
            for search_l in range(l_idx - 1, -1, -1):
                series_in_layer = [nid for nid in layers[search_l]
                                   if nodes[nid].node_type == 'series']
                if series_in_layer:
                    pick = int(rng.choice(series_in_layer))
                    if pick not in child.parents:
                        child.parents = sorted(child.parents + [pick])
                    added = True
                    break

            if not added:
                # Extremely rare: no series node above at all — convert a parent
                if child.parents:
                    nodes[child.parents[0]].node_type = 'series'
